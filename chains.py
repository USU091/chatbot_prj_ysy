...
def create_question_router_chain():
    ...
    system = """You are an expert at routing a user question.
        The vectorstore contains documents related to NIST CVE/CPE specifications and related documentation.
        Return 'yes' if the question is related to the documentation, otherwise return 'no'.
        If you can't determine if the question is related to the documentation, return 'yes'.
...
def create_question_rewrite_chain():
    ...
    system = """You are a question re-writer that converts an input question into a concise search query optimized for NIST CVE/CPE documents in the vectorstore.  
        Preserve canonical IDs (CVE/CPE) if present. If absent, infer vendor/product, version, component, or vulnerability type.  
        Normalize ambiguous tokens and expand abbreviations (e.g., "RCE" → "remote code execution").  
        Add qualifiers for remediation ("patch", "mitigation") or severity ("CVSS") if relevant.  
        Output should be concise, keyword-focused, in English, single line. Do not add explanations or extra content."""
